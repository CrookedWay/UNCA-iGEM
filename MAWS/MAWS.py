...
def get_ligand(topology):
    ...
    for a in topology.atoms():
        if a.residue.name not in ["DGN", "DAN", "DTN", "DCN", "DG", "DA", "DT", "DC", "DG5", "DA5", "DT5", "DC5", "DG3",
                                  "DA3", "DT3", "DC3"]:
...
def get_offset(positions_old, positions):
    ...
    alpha = math.acos(sum([alem.value_in_unit(unit.angstroms) * blem.value_in_unit(unit.angstroms) for alem, blem in
                           zip(vec_a, vec_b)]) / (np.linalg.norm(vec_a.value_in_unit(unit.angstroms)) * np.linalg.norm(
        vec_b.value_in_unit(unit.angstroms))))
    alpha_t = 0.
    # alpha_t = math.pi-113.3*math.pi/360.
    d_alpha = alpha_t - alpha
    axis = cross(vec_a.value_in_unit(unit.angstroms), vec_b.value_in_unit(unit.angstroms)) / np.linalg.norm(
        cross(vec_a.value_in_unit(unit.angstroms), vec_b.value_in_unit(unit.angstroms)))
...
def stratify(pos, energies, size, phi_size):
    ...
    varx1 = np.math.sqrt(
        np.array([np.power(energies[l] - meanx1, 2) for l in range(len(energies)) if pos[l][0] < 0.]).sum() / (
        countx1 - 1 + 1e-20))
    print(varx1)
    vary1 = np.math.sqrt(
        np.array([np.power(energies[l] - meany1, 2) for l in range(len(energies)) if pos[l][1] < 0.]).sum() / (
        county1 - 1 + 1e-20))
    varz1 = np.math.sqrt(
        np.array([np.power(energies[l] - meanz1, 2) for l in range(len(energies)) if pos[l][2] < 0.]).sum() / (
        countz1 - 1 + 1e-20))
    vari1 = np.math.sqrt(
        np.array([np.power(energies[l] - meani1, 2) for l in range(len(energies)) if pos[l][3] < 0.]).sum() / (
        counti1 - 1 + 1e-20))
    varj1 = np.math.sqrt(
        np.array([np.power(energies[l] - meanj1, 2) for l in range(len(energies)) if pos[l][4] < 0.]).sum() / (
        countj1 - 1 + 1e-20))
    vark1 = np.math.sqrt(
        np.array([np.power(energies[l] - meank1, 2) for l in range(len(energies)) if pos[l][5] < 0.]).sum() / (
        countk1 - 1 + 1e-20))
    varphi1 = np.math.sqrt(
        np.array([np.power(energies[l] - meanphi1, 2) for l in range(len(energies)) if pos[l][6] < 0.]).sum() / (
        countphi1 - 1 + 1e-20))
    varx2 = np.math.sqrt(
        np.array([np.power(energies[l] - meanx2, 2) for l in range(len(energies)) if pos[l][0] >= 0.]).sum() / (
        countx2 - 1 + 1e-20))
    vary2 = np.math.sqrt(
        np.array([np.power(energies[l] - meany2, 2) for l in range(len(energies)) if pos[l][1] >= 0.]).sum() / (
        county2 - 1 + 1e-20))
    varz2 = np.math.sqrt(
        np.array([np.power(energies[l] - meanz2, 2) for l in range(len(energies)) if pos[l][2] >= 0.]).sum() / (
        countz2 - 1 + 1e-20))
    vari2 = np.math.sqrt(
        np.array([np.power(energies[l] - meani2, 2) for l in range(len(energies)) if pos[l][3] >= 0.]).sum() / (
        counti2 - 1 + 1e-20))
    varj2 = np.math.sqrt(
        np.array([np.power(energies[l] - meanj2, 2) for l in range(len(energies)) if pos[l][4] >= 0.]).sum() / (
        countj2 - 1 + 1e-20))
    vark2 = np.math.sqrt(
        np.array([np.power(energies[l] - meank2, 2) for l in range(len(energies)) if pos[l][5] >= 0.]).sum() / (
        countk2 - 1 + 1e-20))
    varphi2 = np.math.sqrt(
        np.array([np.power(energies[l] - meanphi2, 2) for l in range(len(energies)) if pos[l][6] >= 0.]).sum() / (
        countphi2 - 1 + 1e-20))
...
def uniform_strat(x_var, y_var, z_var, i_var, j_var, k_var, phi_var, size, phi_size):
    ...
    distx = np.random.choice(
        np.array([np.random.uniform(size[0] + (l - 1) * stepx, size[0] + l * stepx) for l in range(1, len(x) + 1)]),
        p=np.append(x[:-1], [1 - sum(x[:-1])]))
    disty = np.random.choice(
        np.array([np.random.uniform(size[0] + (l - 1) * stepy, size[0] + l * stepy) for l in range(1, len(y) + 1)]),
        p=np.append(y[:-1], [1 - sum(y[:-1])]))
    distz = np.random.choice(
        np.array([np.random.uniform(size[0] + (l - 1) * stepz, size[0] + l * stepz) for l in range(1, len(z) + 1)]),
        p=np.append(z[:-1], [1 - sum(z[:-1])]))
    disti = np.random.choice(np.array(
        [np.random.uniform(phi_size[0] + (l - 1) * stepi, phi_size[0] + l * stepi) for l in range(1, len(i) + 1)]),
                             p=np.append(i[:-1], [1 - sum(i[:-1])]))
    distj = np.random.choice(np.array(
        [np.random.uniform(phi_size[0] + (l - 1) * stepj, phi_size[0] + l * stepj) for l in range(1, len(j) + 1)]),
                             p=np.append(j[:-1], [1 - sum(j[:-1])]))
    distk = np.random.choice(np.array(
        [np.random.uniform(phi_size[0] + (l - 1) * stepk, phi_size[0] + l * stepk) for l in range(1, len(k) + 1)]),
                             p=np.append(k[:-1], [1 - sum(k[:-1])]))
    distphi = np.random.choice(np.array(
        [np.random.uniform(phi_size[0] + (l - 1) * stepphi, phi_size[0] + l * stepphi) for l in
         range(1, len(phi) + 1)]), p=np.append(phi[:-1], [1 - sum(phi[:-1])]))
...
def energy_strata(xyz, energies):
    energies = [math.mpmathify(elem) for elem in energies]
    strata = [[energies[l] for l in range(len(energies)) if
               (c * xyz[l][0] <= 0) and (b * xyz[l][1] <= 0) and (a * xyz[l][2] <= 0) and (d * xyz[l][3] <= 0) and (
               e * xyz[l][4] <= 0) and (f * xyz[l][5] <= 0) and (g * xyz[l][6] <= 0)] for a in [-1, 1] for b in [-1, 1]
              for c in [-1, 1] for d in [-1, 1] for e in [-1, 1] for f in [-1, 1] for g in [-1, 1]]
    return strata
...
class Aptamer:
    ...
    def save_all(self, identifier, identifier1, identifier2, path):
        ...
        self.command(
            "saveamberparm " + identifier + " " + path + identifier + ".prmtop " + path + identifier + ".inpcrd")

    def torsionCN(self, identifier, residue, angle):
        self.command(
            "impose " + identifier + " {" + str(residue) + """} {{ "C2'" "C1'" "N9" "C4" """ + str(angle) + """}}""")
        self.command(
            "impose " + identifier + " {" + str(residue) + """} {{ "C2'" "C1'" "N1" "C6" """ + str(angle) + """}}""")
    ...
    def rotate(self, identifier, angles, step=1):
        ...
        rotateZ = [math.cos(angles[0] * ang_num), -math.sin(angles[0] * ang_num), 0, math.sin(angles[0] * ang_num),
                   math.cos(angles[0] * ang_num), 0, 0, 0, 1]
        rotateY = [math.cos(angles[1] * ang_num), 0, -math.sin(angles[1] * ang_num), 0, 1, 0,
                   math.sin(angles[1] * ang_num), 0, math.cos(angles[1] * ang_num)]
        rotateX = [1, 0, 0, 0, math.cos(angles[2] * ang_num), -math.sin(angles[2] * ang_num), 0,
                   math.sin(angles[2] * ang_num), math.cos(angles[2] * ang_num)]
        antirotateZ = [math.cos(-angles[0] * ang_num), -math.sin(-angles[0] * ang_num), 0,
                       math.sin(-angles[0] * ang_num), math.cos(-angles[0] * ang_num), 0, 0, 0, 1]
        antirotateY = [math.cos(-angles[1] * ang_num), 0, -math.sin(-angles[1] * ang_num), 0, 1, 0,
                       math.sin(-angles[1] * ang_num), 0, math.cos(-angles[1] * ang_num)]
        antirotateX = [1, 0, 0, 0, math.cos(-angles[2] * ang_num), -math.sin(-angles[2] * ang_num), 0,
                       math.sin(-angles[2] * ang_num), math.cos(-angles[2] * ang_num)]
        # self.command("transform "+identifier+""+"{ {%s %s %s} {%s %s %s} {%s %s %s} }"%tuple(rotateZ))
        self.command("transform " + identifier + "." + str(step + 1) + "{ {%s %s %s} {%s %s %s} {%s %s %s} }" % tuple(
            antirotateZ))
    ...
    def rotate_axis(self, identifier, angle, axis, step):
        ...
        # self.command("transform "+identifier+""+"{ {%s %s %s} {%s %s %s} {%s %s %s} }"%tuple(rotateZ))
        self.command(
            "transform " + identifier + "." + str(step + 1) + "{ {%s %s %s} {%s %s %s} {%s %s %s} }" % tuple(rotateM))
...
def ligand_energy():
    ...
    lig_crd = app.AmberInpcrdFile("ligand.inpcrd")
    lig_system = lig_top.createSystem(nonbondedMethod=NoCutoff, nonbondedCutoff=10 * nanometer, constraints=HAngles,
...
def initial_sample(topology, coordinates, Nsteps, index, box=50, rang=[0, 60]):
    ...
    for i in range(Nsteps):
        ...
        shift = mm.Vec3(np.random.uniform(-box, box), np.random.uniform(-box, box),
                        np.random.uniform(-box, box)) * unit.angstrom
        ...
        x, y, z = np.array([x, y, z]) * 1 / (np.linalg.norm(np.array([x, y, z])))
        xyz.append([shift[0].value_in_unit(unit.angstroms), shift[1].value_in_unit(unit.angstroms),
                    shift[2].value_in_unit(unit.angstroms), x, y, z, phi_2])
...
def stratified_sample(topology, coordinates, variances, Nsteps, index, box=50, rang=[0, 400]):
    ...
    for i in range(Nsteps):
        ...
        shift = [0, 0, 0]
        shift[0], shift[1], shift[2], x, y, z, phi_2 = uniform_strat(variances[0], variances[1], variances[2],
                                                                     variances[3], variances[4], variances[5],
                                                                     variances[6], [-box, box],
                                                                     [-np.math.pi, np.math.pi])
        x, y, z = np.array([x, y, z]) * 1 / (np.linalg.norm(np.array([x, y, z])))
...
def mcmc_sample(topology, coordinates, old_coordinates, index, stepsize=200, Nsteps=5000, ligand_heavyness=1e50,
                aptamer_heaviness=1e3):
    aptamer_top = topology
    aptamer_crd = coordinates
    pos = old_coordinates
    # print(len(pos))
    ...
    system = aptamer_top.createSystem(nonbondedMethod=app.CutoffNonPeriodic, nonbondedCutoff=1.2 * unit.nanometers,
                                      constraints=app.HBonds, implicitSolvent=app.OBC1)
    ...
    positions = []
    free_E_old = 1e20
    simulation.context.setPositions(get_C5(pos, get_C5O5(pos, get_base(pos, get_PO5(pos, get_PO3(pos,
                                                                                                 position_aptamer(pos,
                                                                                                                  aptamer_crd.positions)))))))

    state = simulation.context.getState(getPositions=True, getEnergy=True, groups=1)

    for i in range(Nsteps):
        simulation.context.setPositions(get_base(pos, get_C5(pos, get_C5O5(pos, get_PO5(pos, get_PO3(pos,
                                                                                                     position_aptamer(
                                                                                                         pos,
                                                                                                         aptamer_crd.positions)))))))
        state = simulation.context.getState(getPositions=True, getEnergy=True, groups=1)
...
def mcmc_sample_five(topology, coordinates, old_coordinates, index, stepsize=200, Nsteps=20, ligand_heavyness=1e50,
                     aptamer_heaviness=1e3):
    aptamer_top = topology
    aptamer_crd = coordinates
    pos = np.array(old_coordinates)
    heavies = [len(get_ligand(aptamer_top.topology)) + (len(positions) - len(positions_old)) - 1,
               len(aptamer_crd.coordinates)]
    system = aptamer_top.createSystem(nonbondedMethod=app.CutoffNonPeriodic, nonbondedCutoff=1.2 * unit.nanometers,
                                      constraints=app.HBonds, implicitSolvent=app.OBC1)
...
def initial(Ntide):
    ...
    print(Ntide)
    en_pos_xyz = [initial_sample(aptamer_top, aptamer_crd, _NINIT, i, box=sample_box, rang=ligand_range) for i in
                  range(100)]
    ...
    for i in range(_NMER):
        ...
        pos_Nt_S_tasks = pool.map(step,
                                  [[alem, blem.replace("3", "").replace("N", "5").strip() + ntide, 1] for alem, blem in
                                   zip(positions, Ntides) for ntide in [" DG3", " DA3", " DT3", " DC3"]])
